...
import io.vertx.guides.wiki.step05.database.WikiDatabaseService
import io.vertx.guides.wiki.step05.database.createProxy
...
class HttpServerVerticle : AbstractVerticle() {
    ...
    override fun start(startFuture: Future<Void>) {
        ...
        router.get("/backup").handler(::backupHandler)
    ...
    private fun indexHandler(context: RoutingContext) {
        dbService?.fetchAllPages(Handler { reply ->
            if (reply.succeeded()) {
                ...
                templateEngine.render(context, "templates", "/index_05.ftl") { ar ->
                    if (ar.succeeded()) {
